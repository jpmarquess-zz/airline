            ...
            <tbody>
                <?php foreach($reservas as $reserva) : ?>
                    <?php 
                        ...
                        $output .= "<td class='align-middle'>". $reserva['origem'] ."</td>";
                        $output .= "<td class='align-middle'>". $reserva['destino'] ."</td>";
                        $output .= "<td class='align-middle'>". $reserva['valor'] ." €</td>";
                        $output .= "<td>";
                        $output .= "<div class='actions-wrapper'>";
                        $output .= form_open('voos/edit/' . $reserva['reserva_id']);
                        $output .= "<button type='submit' class='btn btn-info mr-3'>Edit</button>";
                        $output .= form_close();
                        $output .= form_open('voos/delete/' . $reserva['reserva_id']);
